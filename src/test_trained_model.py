...
class VPRTester:
    ...
    def __init__(self, root, data_conf, data_folders, vpr_conf, train_conf, model_IO):
        ...
        log_dir = join(root, model_IO["logs_path"], data_conf["name"] + "_test", "res_" + data_conf["test_res"], log_suffix,
                       "trained_descs" if data_conf["use_trained_descs"] else "pretrained_descs")
        ...
        self.database_images_set = self.load_database_images(data_folders, pretrained_suffix, trained_suffix, data_conf["use_trained_descs"])
    ...
    def load_database_images(self, data_folders, pretrained_suffix, trained_suffix, use_trained_descs):
...
if __name__=='__main__':
    ...
    if args.loss_num is not None:
        ...
        for enabled, loss_type in zip(loss_combs[args.loss_num - 1], config["train_conf"]["loss"]):
